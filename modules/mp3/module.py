...
class mp3(QtGui.QMainWindow, form_class):
    ...
    def play(self):
        self.parent.player = self.__class__.__name__
        if self.settings["pause"]==0:
    ...
    def pullInfo(self):
        print(self.lbl_Title.fontMetrics().width(self.lbl_Title.text()))
    ...
    def like(self):
        ...
        if self.lbl_Title.text() != "":
            ...
            if like:
                ...
                self.btn_Like.setStyleSheet("background-image: url(./images/like.png);background-repeat: none; background-color: #eeeeee; border-radius: 5px; border: 0px;background-position: center")
            else:
                ...
                self.btn_Like.setStyleSheet("background-image: url(./images/liked.png);background-repeat: none; background-color: #eeeeee; border-radius: 5px; border: 0px;background-position: center")
...
########################################################################
    ...
    def changeBackground(self, f, title):
        try:
            self.getAlbumCover(f, "/tmp/bg.jpg")
            source = Image.open("/tmp/bg.jpg")
            source=source.filter(ImageFilter.GaussianBlur(4))
            if source.width<700:
                width=700
                height=int(width * (source.height / source.width))
            ...
            Blue = im[B].point(lambda i: i/constant)
            source = Image.merge(source.mode, (Red, Green, Blue))
            os.remove("/tmp/bg.jpg")
            source.save("/tmp/background.jpg")
            image="/tmp/background.jpg"
        except:
            print("Unexpected error:", sys.exc_info()[0])
            image=path[0]+"/"+path[1]+"/skin.png"        
        return image
    ...
    def getSongs(self):
        if self.albumList.currentRow()>-1:
            ...
            for song in sorted(self.music[album]):
                ...
                try:
                    artwork = file.tags['APIC:'].data # access APIC frame and grab the image
                    with open('/tmp/cover'+tstamp+'.jpg', 'wb') as img:
                        img.write(artwork) # write artwork to new image
                        image="/tmp/cover"+tstamp+".jpg"
                except:
    ...
    def getMusic(self):
        ...
        for album in sorted(music):
            ...
            if album==self.settings["currentAlbum"]:
                ...
                self.albumList.setCurrentRow(z)
                if self.settings["currentSong"]!="":
                    self.lbl_Artist.setText(music[album][self.settings["currentSong"]]["artist"])
                    self.lbl_Title.setText(music[album][self.settings["currentSong"]]["title"])
                    self.player.setStyleSheet("background-image: url("+ self.changeBackground(music[album][self.settings["currentSong"]]["path"], music[album][self.settings["currentSong"]]["title"]) +");")
                    mp3player.loadfile(music[album][self.settings["currentSong"]]["path"]);
                    mp3player.pause()
                    self.lbl_Time.setText(self.getSongLength(mp3player.length))
                    self.musicSlider.setMaximum(int(mp3player.length))
                    if self.settings["position"]!="":
                        mp3player.time_pos=self.settings["position"]
                        self.musicSlider.setValue(self.settings["position"])
                    else:
                        self.musicSlider.setValue(0)
            z+=1
