...
class QPaeq(QtWidgets.QWidget):
    ...
    def create_layout(self):
        ...
        self.main_layout.setGeometry(QtCore.QRect(0,0, 651, 171))
    ...
    def connect_to_sink(self,name):
        ...
        self.filter_state=FilterState(sink, self, self.parent)
    ...
    def save_profile(self, name):
        ...
        self.sink.SaveProfile(self.filter_state.channel,dbus.String(name))
        if self.filter_state.channel==self.filter_state.channels:
            for x in range(1,self.filter_state.channels):
                self.sink.LoadProfile(x,dbus.String(name))
    ...
    def load_profile(self,name):
        try:
            self.filter_state.load_profile(name)
        except:
            self.reset()
        """
...
class SliderArraySub(QtWidgets.QWidget):
    def __init__(self,filter_state,parent=None):
        ...
        #self.setStyleSheet('padding: 0px; border-width: 2px; margin: 0px;')
        #self.setStyleSheet('font-family: monospace;'+outline%('blue'))
        qt=QtCore.Qt
        #self.layout().setHorizontalSpacing(1)
        def add_slider(slider,label, c):
            #slider.setStyleSheet("border: 1px solid #000000; background-color: #ff0")
...
class FilterState(QtCore.QObject):
    ...
    def __init__(self,sink, parent, pyCAR):
    ...
    def set_filter(self,preamp,coefs):
        ...
        self.sync_timer.start(SYNC_TIMEOUT)
    def save_state(self):
        print('saving state for', self.pyCAR.player)
        self.sink.SaveState()
        self.parent.save_profile(self.pyCAR.player)
